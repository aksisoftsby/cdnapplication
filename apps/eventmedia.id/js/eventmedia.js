...
jQuery(document).ready(() => {
	fetch('https://www.instagram.com/eventmedia.id/?__a=1')
		.then(response => response.json())
		.then(j => {
			if (typeof j.graphql !== 'undefined') {
				if (typeof j.graphql.user.edge_owner_to_timeline_media.edges !== 'undefined') {
					var edg = j.graphql.user.edge_owner_to_timeline_media.edges;
					var uledge = jQuery('ul.insta-feed');
					jQuery.each(edg, function (a, b) {
						uledge
							.append($('<li>')
								.append($('<a>', {
									href: 'https://www.instagram.com/p/' + b.node.shortcode,
									target: '_blank',
									'data-animate': 'zoomIn',
									'data-duration': '1.5s',
									'data-delay': '0.1s',
									...
									src: b.node.thumbnail_src,
									'class': 'border-radius-5',
									'alt': "Instagram eventmedia.id"
								}))));
					})
				}
			}
